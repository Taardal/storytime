...
namespace Storytime {
    ...
    void VulkanDescriptorSet::bind(const VulkanCommandBuffer& command_buffer, const BindConfig& config) const {
        bind(command_buffer, config.pipeline_bind_point, config.pipeline_layout, config.set);
    }
