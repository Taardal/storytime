...
void print_lua_stack(lua_State* L, const std::string& tag) {
    ...
    ST_ASSERT(
        ...
        "Lua stack size after printing [" << current_stack_size << "] must equal before printing [" << stack_size << "]"
    );
}
...
std::string lua_table_to_string(lua_State* L, const int table_index, const int max_depth) {
    ...
    ST_ASSERT(
        type == LUA_TTABLE,
        "Cannot stringify Lua table: Item on stack index [" << table_index << "] is of type [" << type_name << "]"
    );
