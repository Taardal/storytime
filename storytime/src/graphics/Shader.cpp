...
namespace Storytime
{
    ...
    {
        ST_GL_CALL(int32_t location = glGetUniformLocation(id, key));
    ...
    uint32_t Shader::CreateShader(const char* source, uint32_t type) const
    {
        ...
        ST_GL_CALL(uint32_t shaderId = glCreateShader(type));
        ...
        else
        {
            ...
            ST_GL_CALL(glDeleteShader(shaderId));
            return 0;
        }
    }
    ...
    void Shader::SetShaderSource(const char* source, uint32_t shaderId) const
    {
        ...
        ST_GL_CALL(glShaderSource(shaderId, count, &source, length));
    ...
    bool Shader::CompileShader(uint32_t shaderId) const
    {
        ...
        ST_GL_CALL(glCompileShader(shaderId));
        int32_t status = 0;
        ST_GL_CALL(glGetShaderiv(shaderId, GL_COMPILE_STATUS, &status));
    ...
    {
        int32_t length = 0;
        ST_GL_CALL(glGetShaderiv(shaderId, GL_INFO_LOG_LENGTH, &length));
        std::vector<char> log(length);
        ST_GL_CALL(glGetShaderInfoLog(shaderId, length, &length, &log[0]));
        return log.data();
    }
    ...
    {
        int32_t logLength = 0;
        ST_GL_CALL(glGetProgramiv(programId, GL_INFO_LOG_LENGTH, &logLength));
        std::vector<char> log(logLength);
        ST_GL_CALL(glGetProgramInfoLog(programId, logLength, &logLength, &log[0]));
        return log.data();
    }
